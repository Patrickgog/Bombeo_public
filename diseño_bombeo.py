...
class CentrifugalPumpDesigner:
    ...
    def __init__(self, q_design, h_static, rpm, hourly_factors, pipe_length_m, pipe_diameter_m, 
                 ...
                 initial_tank_level_perc, simulation_days=1, tank_capacity_m3=None):
        # Parámetros básicos
        self.q_design = q_design  # m3/s
        self.h_static = h_static  # m
        self.rpm = rpm
        self.hourly_factors = np.array(hourly_factors, dtype=float)  # <-- conversión robusta
        ...
        self.initial_tank_level_perc = initial_tank_level_perc
        self.simulation_days = simulation_days
        self.tank_capacity_m3 = tank_capacity_m3
        
        # Calcular parámetros derivados
        self.q_range = np.linspace(1e-6, 1.5 * q_design, 2000)
        self._calculate_operating_point()
        self._calculate_vfd_results()
        self._calculate_tank_simulation()
    ...
    def _calculate_operating_point(self):
        ...
        pump_head_curve = self._pump_head(self.q_range)
        
        # La curva del sistema depende del caudal total
        system_head_curve = self.h_static + self._friction_loss(q_total)
        ...
        self.q_op = float(q_op_per_pump)
        self.h_op = float(h_op)
    
    def _friction_loss(self, q):
        """Pérdidas por fricción usando Hazen-Williams"""
        # hf = 10.67 * L * Q^1.852 / (C^1.852 * D^4.871)
        q = np.asarray(q)
        # Protección contra división por cero
        return np.where(q == 0, 0, 10.67 * self.pipe_length_m * (q**1.852) / (self.hw_c**1.852 * self.pipe_diameter_m**4.871))
    
    def _pump_head(self, q):
        """Curva H-Q de la bomba (m), modelada como H = H_shutoff - a_p * Q^2."""
        # Calcular coeficientes de la bomba
        h_design = self.h_static + self._friction_loss(self.q_design)
        h_shutoff = 1.3 * h_design  # Carga de cierre ~130% de la de diseño
        a_p = (h_shutoff - h_design) / self.q_design**2 if self.q_design > 1e-9 else 0.0
        return h_shutoff - a_p * np.asarray(q, dtype=float)**2
    
    def _calculate_vfd_results(self):
        """Calcular resultados con VFD"""
        # Simular VFD para ajustar el punto de operación
        speed_ratio = 0.85  # 85% de velocidad
        q_vfd = self.q_op / speed_ratio
        h_vfd = self.h_op / (speed_ratio**2)
        ...
        self.vfd_results = {
            ...
            'h_op': h_vfd,
            'total_power_kw': self.power_kW(self.q_op) * self.n_parallel * speed_ratio**3,
            'q_min_lps': self.q_op * 0.5 * 1000,
            'q_max_lps': self.q_op * 1.5 * 1000
        }
    
    def _calculate_tank_simulation(self):
        """Simular comportamiento del tanque"""
        # Asegurar que hourly_factors sea un array de floats antes de cualquier multiplicación
        if not isinstance(self.hourly_factors, np.ndarray) or self.hourly_factors.dtype != float:
            try:
                self.hourly_factors = np.array(self.hourly_factors, dtype=float)
            except Exception:
                # Si falla la conversión, usar factores por defecto
                self.hourly_factors = np.array([1.0]*24, dtype=float)
        
        if self.tank_capacity_m3 is None:
            # Calcular capacidad automáticamente usando método Rippl
            ...
            self.tank_capacity_m3 = float(max(cap, 50.0))
        
        # Simulación horaria usando método de pump_design.py
        ...
        vol[0] = self.tank_capacity_m3 * self.initial_tank_level_perc
        ...
        self.demand_flow = q_demand / 3600  # m3/s para el gráfico
        self.pump_flow = np.where(pump_on > 0, q_supply / 3600, 0)  # m3/s
    
    def power_kW(self, q):
        """Potencia eje (kW) para caudal q."""
        head = self._pump_head(q)
    ...
    def efficiency(self, q):
        ...
        # Protección contra división por cero
        if self.q_design <= 1e-9:
            return np.full_like(q, self.eff_peak)
        
        eta = self.eff_peak * (1 - ((q - self.q_design) / self.q_design)**2)
        return np.clip(eta, 0, None)
    
    def affinity_curves(self, rpm_list):
        """Calcula las curvas de afinidad para diferentes velocidades"""
        curves = {}
        for rpm in rpm_list:
            ratio = rpm / self.rpm
            q_scaled = self.q_range * ratio
            h_scaled = self._pump_head(self.q_range) * ratio**2
            curves[rpm] = (q_scaled, h_scaled)
        return curves
    
    # --- MÉTODOS DE GRÁFICOS ---
    def _apply_plotly_layout(self, fig, title, xaxis_title, yaxis_title):
    ...
    def system_head(self, q):
        """Curva del sistema (H-Q) usando Hazen-Williams."""
        return self.h_static + self._friction_loss(q)

    def pump_head(self, q):
        """Curva H-Q de la bomba (m), modelada como H = H_shutoff - a_p * Q^2."""
        return self._pump_head(q)

    def plot_system_vs_pump(self):
        """Genera un gráfico interactivo de las curvas H-Q del sistema y las bombas (idéntico a pump_design.py)."""
        fig = go.Figure()
    ...
    def plot_vfd_comparison(self):
        """
        Genera un gráfico comparando la operación a velocidad nominal vs. con VFD (punto magenta en la intersección real y potencia calculada en ese punto).
        """
        if not hasattr(self, 'vfd_results') or not self.vfd_results:
            return None
        fig = go.Figure()
        r = 0.892  # 89.2% fijo como en tu referencia
        q_total_range_lps = self.q_range * self.n_parallel * 1000
        # Curva combinada 100% RPM
        ...
        # Curva combinada VFD (naranja, 89.2%)
        ...
        fig.add_trace(go.Scatter(x=q_total_range_lps, y=combined_pump_head_vfd, mode='lines', name=f'Curva Combinada @ 89.2% RPM', line=dict(color='orange', width=3, dash='dash')))
        # Curva del sistema (azul)
        system_head_curve = self.system_head(self.q_range * self.n_parallel)
        fig.add_trace(go.Scatter(x=q_total_range_lps, y=system_head_curve, mode='lines', name='Curva del Sistema', line=dict(color='blue', width=3)))
        # Calcular la intersección real
        diff = np.abs(combined_pump_head_vfd - system_head_curve)
        idx = np.argmin(diff)
        q_op_vfd_total_lps = q_total_range_lps[idx]
        h_op_vfd = system_head_curve[idx]
        q_op_vfd_bomba = self.q_range[idx]  # caudal por bomba en m3/s
        # Potencia en ese punto
        eta_vfd = self.efficiency(q_op_vfd_bomba)
        if eta_vfd < 0.01:
            eta_vfd = self.eff_peak if self.eff_peak > 0.01 else 0.78
        power_vfd = (self.RHO_WATER * self.G * q_op_vfd_bomba * h_op_vfd / eta_vfd) / 1000 * self.n_parallel
        # Punto de operación VFD (magenta, en la intersección)
        fig.add_trace(go.Scatter(x=[q_op_vfd_total_lps], y=[h_op_vfd], mode='markers+text', name=f'OP @ 89.2% RPM',
    ...
    def plot_pump_operating_range(self):
        """Muestra el rango operacional de una bomba individual y su punto de operación VFD (idéntico a pump_design.py)."""
        if not hasattr(self, 'vfd_results') or not self.vfd_results:
            return None
        fig = go.Figure()
        # Curva bomba individual
    ...
    def plot_total_power_analysis(self):
        ...
        if hasattr(self, 'vfd_results') and self.vfd_results:
    ...
    def plot_cost_per_m3_vs_flow(self):
        """Gráfico: Costo unitario vs caudal"""
        fig = go.Figure()
    ...
    def plot_power_affinity(self):
        """Gráfico: Potencia vs caudal (afinidad)"""
        fig = go.Figure()
    ...
    def plot_efficiency_affinity(self):
        """Gráfico: Eficiencia vs caudal (afinidad)"""
        fig = go.Figure()
        rpm_list = [self.rpm * r for r in np.linspace(1, 0.5, 6)]
        for n in rpm_list:
            ratio = n / self.rpm
    ...
    def plot_tank_volume(self):
        """Gráfico: Volumen en tanque"""
        fig = go.Figure()
        num_hours = 24 * self.simulation_days
        hrs_vol = np.arange(num_hours + 1)
        
        fig.add_trace(go.Scatter(x=hrs_vol, y=self.tank_volume, name='Volumen Tanque (m³)',
    ...
    def plot_demand_vs_inflow(self):
        ...
        demand_lps = self.demand_flow * 1000
...
def get_input_panel(dias=None):
    ...
    campos = [
        ...
        ("% RPM curva combinada:", dcc.Input(id='input-rpm-combinada', type='number', value=100, step=0.1, min=50, max=100, style={'width': '120px'}), 'input-rpm-combinada'),
    ...
    return html.Div([
        ...
        *filas,
        html.Div([
            ...
            html.Button('Cerrar', id='btn-close-panel', n_clicks=0, style={'marginTop': '18px', 'fontFamily': 'Segoe UI', 'fontSize': '15px', 'width': '120px', 'background': '#eee', 'color': '#333', 'border': 'none', 'borderRadius': '6px', 'display': 'inline-block'})
...
def update_dashboard(n_clicks, Q_lps, h_static, rpm, length, diam_mm, n_parallel, hw_c, eff_peak, elec_cost, tank_init, tank_min, days, hourly_str, rpm_combinada, tank_cap, show_analysis):
    import traceback
    try:
        ...
        for tab_idx, tab in enumerate(tabs_graficos):
            ...
            for i in tab:
                ...
                if i == 1:
                    ...
                    q_homologous_100 = q_per_pump_vfd / r
                    h_range_edit = designer._pump_head(q_homologous_100) * r**2
                ...
                if i == 2:
                    ...
                    h_range_azul = designer._pump_head(q_homologous_100) * r**2
                    fig_filtrada.add_trace(go.Scatter(
                    ...
                    if designer.vfd_results:
                        ...
                        h_range_naranja = designer._pump_head(q_homologous_100_vfd) * r_vfd**2
                        fig_filtrada.add_trace(go.Scatter(
                            x=q_range_lps * r_vfd,
                            y=h_range_naranja,
                            mode='lines',
